...
1. Поверх страницы мгновенно накладывается изолированный терминальный экран осознанности (Shadow DOM) с 60 FPS дыхательной анимацией (`Вдох -> Задержка -> Выдох -> Покой`).
2. Всякое видео и аудио на странице автоматически глушится, а взаимодействие с сайтом блокируется.
3. Кнопка **«ВЫЙТИ»** доступна мгновенно — при нажатии закрывает вкладку и увеличивает счетчик сбереженного времени.
4. Кнопка **«ПРОДОЛЖИТЬ НА САЙТ»** появляется после завершения дыхания: оверлей плавно растворяется, а **сайт сразу готов к использованию без повторной загрузки и потери состояния**!
