...
class Migration6To7Test {
    ...
    @Test
    fun migrationPreservesExistingTargetAndUsesAnnoyingUnlockDefaults() = runTest {
        val context = ApplicationProvider.getApplicationContext<Context>()
        val name = "annoying-unlock-migration.db"
        context.deleteDatabase(name)
        val schemaFile = listOf(
            File("schemas/io.ronesec.android.data.WattimDatabase/6.json"),
            File("app/schemas/io.ronesec.android.data.WattimDatabase/6.json")
        ).first { it.exists() }
        ...
        // Schema 6 columns: 24 columns
        helper.writableDatabase.execSQL(
            "INSERT INTO target_apps VALUES ('sample.app', 'Sample', 1, 'Breathe', 'FILL', 8000, 300000, 0, 0, 20, 3600000, 7, 1, 6, 1, 1, 12000, 1, 3, 0, 1, 1, 5, 6000)"
        )
        helper.close()

        val database = Room.databaseBuilder(context, WattimDatabase::class.java, name)
            .addMigrations(WattimDatabase.MIGRATION_6_7, WattimDatabase.MIGRATION_7_8).allowMainThreadQueries().build()
        try {
            ...
            assertFalse(target.annoyingUnlockEnabled)
            assertEquals(20, target.annoyingUnlockChancePercent)
        } finally {
            database.close()
            context.deleteDatabase(name)
        }
